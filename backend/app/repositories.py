...
from app.models import User, Patient, Doctor, Specialty, Schedule, Medicine, Prescription, PrescriptionDetail, \
    Appointment, MedicalRecord, Examination, ScheduleStatus, ScheduleSlot
...
class DoctorRepository(BaseRepository[Doctor]):
    ...
    def get_by_specialty(self,db: Session,specialty_id: int,*,skip: int = 0,limit: int = 100) -> list[Doctor]:
        return (db.query(Doctor).filter(Doctor.specialty_id == specialty_id,Doctor.status == "active").
    ...
    def get_active_doctors_by_specialty(self,db: Session,specialty_id: int) -> list[Doctor]:
        return db.query(Doctor).filter(Doctor.status == "active",Doctor.specialty_id == specialty_id).all()

    def get_active_by_id(self,db: Session,doctor_id: int) -> Optional[Doctor]:
        return db.query(Doctor).filter(Doctor.id == doctor_id,Doctor.status == "active").first()
    ...
    def count_active_by_specialty(self,db: Session,specialty_id: int) -> int:
        return (db.query(Doctor).filter(Doctor.specialty_id == specialty_id,Doctor.status == "active").count())

    def exists_active_by_specialty(self,db: Session,specialty_id: int) -> bool:
        return db.query(Doctor).filter(Doctor.specialty_id == specialty_id,Doctor.status == "active").first() is not None
...
class ScheduleRepository(BaseRepository[Schedule]):
    ...
    def get_open_schedules(self,db: Session,*,skip: int = 0,limit: int = 100):
        return db.query(Schedule).filter(Schedule.status == ScheduleStatus.OPEN).offset(skip).limit(limit).all()
...
class AppointmentRepository(BaseRepository[Appointment]):
    ...
    def get_by_doctor(self, db: Session, doctor_id: int, *, skip: int = 0, limit: int = 100) -> list[Appointment]:
        return db.query(Appointment).join(Appointment.slot).join(ScheduleSlot.schedule).filter(Schedule.doctor_id == doctor_id).offset(skip).limit(limit).all()
...
class MedicalRecordRepository(BaseRepository[MedicalRecord]):
    ...
    def exists_by_patient(self,db: Session,patient_id: int) -> bool:
        return db.query(MedicalRecord).filter(MedicalRecord.patient_id == patient_id).first() is not None
...
class ExaminationRepository(BaseRepository[Examination]):
    ...
    def get_by_medical_record(self,db: Session,medical_record_id: int):
        return db.query(Examination).filter(Examination.medical_record_id== medical_record_id).all()
...
class MedicineRepository(BaseRepository[Medicine]):
    ...
    def get_active_by_code(self,db: Session,code: str):
        return db.query(Medicine).filter(Medicine.code == code,Medicine.status == "active").first()

    def get_active_by_id(self,db: Session,medicine_id: int):
        return db.query(Medicine).filter(Medicine.id == medicine_id,Medicine.status == "active").first()
