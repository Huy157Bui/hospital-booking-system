...
ScheduleRepoDep = Annotated[
    ScheduleRepository,
    Depends(get_schedule_repository),
]
...
def get_schedule_slot_repository(
    db: DbDep,
) -> ScheduleSlotRepository:
...
ScheduleSlotRepoDep = Annotated[
    ScheduleSlotRepository,
    Depends(get_schedule_slot_repository),
]
...
MedicalRecordRepoDep = Annotated[
    MedicalRecordRepository,
    Depends(get_medical_record_repository),
]


def get_examination_repository(
    db: DbDep,
) -> ExaminationRepository:
...
ExaminationRepoDep = Annotated[
    ExaminationRepository,
    Depends(get_examination_repository),
]
...
def get_prescription_detail_repository(
    db: DbDep,
) -> PrescriptionDetailRepository:
...
PrescriptionDetailRepoDep = Annotated[
    PrescriptionDetailRepository,
    Depends(get_prescription_detail_repository),
]
...
MedicineRepoDep = Annotated[
    MedicineRepository,
    Depends(get_medicine_repository),
]
