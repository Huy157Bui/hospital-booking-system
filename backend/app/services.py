    ...
    async def search(
        ...
        if doctor_name:
            ...
            doctors = [
                ...
                if d_name_lower in (
                    getattr(d, "full_name", "")
                    or (d.user.full_name if getattr(d, "user", None) else "")
                ).lower()
            ]
            if not doctors:
                return [], False
        ...
        if specialty_name:
            ...
            doctors = [
                d for d in doctors
                if getattr(d, "specialty", None) and d.specialty.name
                and (spec_lower in d.specialty.name.lower() or d.specialty.name.lower() in spec_lower)
            ]
            if not doctors:
                return [], False
        ...
        if match:
            ...
            if 2 <= len(words) <= 5:
                if not any(
                    ...
                    for k in ["bạch mai", "trung tâm", "khoa", "bệnh viện", "phòng"]
                ):
                    return clean_name
        return None
